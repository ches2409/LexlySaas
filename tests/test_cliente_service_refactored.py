...
def mock_supabase_port():
    """Fixture que provee un puerto mock de Supabase"""
    port = MagicMock()
    port.get_one = MagicMock()
    port.get_all = MagicMock()
    port.insert = MagicMock()
    port.update = MagicMock()
    port.delete = MagicMock()
    return port
...
def test_get_all_calls_supabase_get_all(service, mock_supabase_port):
    ...
    result = service.get_all("user123")
...
def test_create_calls_supabase_insert(service, mock_supabase_port):
    ...
    result = service.create(data, "user123")
...
def test_update_calls_supabase_update(service, mock_supabase_port):
    """Test que update llama al método update del puerto"""
    mock_supabase_port.get_one.return_value = {"id": "1", "nombre": "Juan"}
    mock_supabase_port.update.return_value = {"id": "1", "nombre": "Juan Actualizado"}

    result = service.update("1", {"nombre": "Juan Actualizado"}, "user123")

    mock_supabase_port.update.assert_called_once_with("clientes", {"id": "1"}, {"nombre": "Juan Actualizado"})


def test_update_returns_none_when_not_found(service, mock_supabase_port):
    """Test que update retorna None cuando no encuentra el cliente"""
    mock_supabase_port.get_one.return_value = None

    result = service.update("nonexistent", {"nombre": "Nuevo"}, "user123")

    assert result is None


def test_delete_calls_supabase_delete(service, mock_supabase_port):
...
def test_delete_returns_false_when_not_found(service, mock_supabase_port):
    """Test que delete retorna False cuando no encuentra el cliente"""
    mock_supabase_port.get_one.return_value = None

    result = service.delete("nonexistent", "user123")
